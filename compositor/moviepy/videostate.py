...
class VideoState:
    ...
    def resize_and_crop_video(self, clip):
        # Decide on video size
        # print("clip size ", str(clip.size))
        # print("frame size ", self.frame_size)
        new_size = XY(random.randrange(min_size.x, min(clip.size[0], self.frame_size.x)),
                      random.randrange(min_size.y, min(clip.size[1], self.frame_size.y)))
        ...
        clip = clip.resize(final_size.get_xy())
